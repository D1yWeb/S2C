...
export const consumeCredits = mutation({
  ...
  handler: async (ctx, { userId, amount, reason, idempotencyKey }) => {
    ...
    const newTotalBalance = (sub ? sub.creditsBalance - Math.min(subscriptionBalance, amount) : 0) + 
                           (purchasedBalance - Math.min(purchasedBalance, remaining + (amount - remaining)));
