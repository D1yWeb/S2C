...
export const useStyleGuide = (
  ...
  const handleGenerateStyleGuide = async () => {
    ...
    try {
      ...
      if (!result.success) {
        const errorMsg = result.message || 'Failed to generate style guide'
        toast.error(errorMsg, { id: 'style-guide-generation' })
        return
      }
      ...
      console.error('Style guide generation error:', errorDetails)
      ...
      if (error?.data?.error) {
        ...
        if (error?.data?.details) {
          errorMessage = `${error.data.error}: ${error.data.details}`
        }
